...
void Texture::CreateTexture(const wstring& path)
{
	...
	HRESULT hr = ::CreateTexture(core->GetDevice().Get(), _image.GetMetadata(), &_tex2D);
...
void Texture::CreateView()
{
	...
	core->GetDevice()->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&_srvHeap));
	...
	core->GetDevice()->CreateShaderResourceView(_tex2D.Get(), &srvDesc, _srvHandle);
}